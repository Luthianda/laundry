<?php
$selectOrder = mysqli_query($config, "SELECT order_date FROM trans_order ORDER BY order_date");
...
$queryReport = mysqli_query($config, "SELECT c.customer_name, o.order_date, s.service_name, od.qty, s.price, od.subtotal FROM trans_order_detail od LEFT JOIN type_of_service s ON od.id_service = s.id LEFT JOIN trans_order o ON od.id_order = o.id LEFT JOIN customer c ON o.id_customer = c.id WHERE o.order_date BETWEEN '$start' AND '$end' ORDER BY od.id, o.total DESC");
